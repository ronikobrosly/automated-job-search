...
class JobOperations:
    ...
    @staticmethod
    def get_job_by_id_and_website(
        session: Session, job_id: str, job_website: str
    ) -> Optional[Job]:
        ...
        return (
            session.query(Job)
            .filter(and_(Job.job_id == job_id, Job.job_website == job_website))
            .first()
        )
    ...
    def get_new_jobs_since(session: Session, since: datetime) -> List[Job]:
        ...
        return (
            session.query(Job)
            .filter(and_(Job.is_new == True, Job.when_scraped >= since))
            .all()
        )

    @staticmethod
    def get_jobs_by_website(
        session: Session, website: str, limit: Optional[int] = None
    ) -> List[Job]:
    ...
    @staticmethod
    def mark_job_as_relevant(
        session: Session, job_id: int, is_relevant: bool = True
    ) -> Optional[Job]:
    ...
    def mark_job_as_processed(
        session: Session, job_id: int, is_processed: bool = True
    ) -> Optional[Job]:
    ...
    def get_unprocessed_relevant_jobs(session: Session) -> List[Job]:
        ...
        return (
            session.query(Job)
            .filter(and_(Job.is_relevant == True, Job.is_processed == False))
            .all()
        )
    ...
    def cleanup_old_jobs(session: Session, days_old: int = 30) -> int:
        ...
        deleted_count = (
            session.query(Job)
            .filter(and_(Job.when_scraped < cutoff_date, Job.is_relevant == False))
            .delete()
        )
        session.commit()
    ...
    def get_job_stats(session: Session) -> Dict:
        ...
        websites = (
            session.query(Job.job_website, func.count(Job.id))
            .group_by(Job.job_website)
            .all()
        )
