...
class SeleniumMixin:
    ...
    def _selenium_get_page(
        self, url: str, wait_for_element: str = None, wait_time: int = 10
            ...
            if wait_for_element:
                try:
                    ...
                    try:
                        ...
                        logging.info(
                            "React root has content, waiting additional time for job listings"
                        )
                        # Additional wait for job content to load
                        time.sleep(5)
                    except TimeoutException:
                        logging.warning(
                            "React root check failed, trying generic body wait"
                        )
                        ...
                        try:
                            ...
                            logging.warning(
                                "Even body element not found, proceeding anyway"
                            )
...
class AntiDetectionMixin:
    ...
    def _get_random_headers(
        self, base_headers: Dict[str, str] = None
    ) -> Dict[str, str]:
    ...
    def _make_request(
        self, url: str, headers: Dict[str, str] = None, timeout: int = 30
        ...
        try:
            ...
            if response.status_code in [403, 429, 503]:
                logging.warning(
                    f"Potential blocking detected on {url}. Status: {response.status_code}"
                )
                return None
    ...
    def _make_detail_request(
        self, url: str, delay_range: tuple = None
    ) -> Optional[requests.Response]:
...
class SeleniumBaseScraper(SeleniumMixin, AntiDetectionMixin, ABC):
        ...
        try:
            ...
            while current_page <= max_pages:
                try:
                    ...
                    if not jobs:
                        self.logger.warning(
                            f"No jobs found on page {current_page}, stopping"
                        )
                        break
                    ...
                    self.logger.error(
                        f"Unexpected error on page {current_page}: {str(e)}"
                    )
                    self.stats["errors"] += 1
                    current_page += 1
    ...
    def extract_job_details(self, job_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            soup = self._selenium_get_page(
                job_url, wait_for_element=self._get_detail_wait_element()
            )
