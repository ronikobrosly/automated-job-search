        ...
        for site_name, site_config in enabled_sites.items():
            try:
                ...
                if scraper:
                    ...
                    self.logger.warning(f"No scraper implementation found for {site_name}")
                ...
                for job_batch in scraper.scrape_all_pages():
                    ...
                    for job_data in job_batch:
                        try:
                            ...
                            self.logger.error(f"Error processing job from {site_name}: {str(e)}")
                            site_stats['errors'] += 1
            ...
            return [
                {
                    ...
                    'when_scraped': job.when_scraped.isoformat() if job.when_scraped else None
                }
                for job in new_jobs
            ]
        finally:
